...
public class WalletManageActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onResume() {
        ...
        if (walletInfos == null || manageListsAdapter == null || walletInfos.size() <= 0) return;
        ...
        manageListsAdapter.setOnItemClickListener(new BaseAdapter.OnItemClickListener() {
            ...
            public void onItemClicked(RecyclerView.Adapter adapter, Object data, int position) {
                if (data instanceof WalletInfo) {
                    WalletDetailActivity.startWalletDetailActivity(WalletManageActivity.this, ((WalletInfo) data).getWalletAddress(), false);
                }
            }
        });
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.rl_create_wallet:
                AnalyticsManager.getInstance().sendEvent(AnalyticsEvent.WalletManagement_button_CreateWallet);

                WalletCreateActivity.startActivity(this, ACTION_SELECT_WALLET);
                break;
            case R.id.rl_import_wallet:
                AnalyticsManager.getInstance().sendEvent(AnalyticsEvent.WalletManagement_button_Importwallet);
