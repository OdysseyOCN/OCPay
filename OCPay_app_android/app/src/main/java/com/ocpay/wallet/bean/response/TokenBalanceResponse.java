...
public class TokenBalanceResponse {
    ...
    public BigDecimal getTokenBalance() {
        ...
        if (result.startsWith("0x") && result.length() == 66) {
            ...
            BigDecimal divide = ethbal.divide(new BigDecimal(1000000000000000000d), ETH_SCALE, BigDecimal.ROUND_DOWN);
            return divide;
        }
    ...
    public BigDecimal getEthBalance() {
        if (!result.startsWith("0x") && !StringUtil.isEmpty(result)) {
            try {
                return new BigDecimal(result).divide(new BigDecimal(1000000000000000000d), 5, BigDecimal.ROUND_DOWN);
            } catch (Exception e) {
                e.printStackTrace();
                return new BigDecimal(0);
            }
