...
public class ContactsCreateActivity extends BaseActivity implements View.OnClickListener {
    ...
    public static final String C_ADDRESS = "contact_address";
    public static final int EDIT = 1;
    public static final int CREATE = 0;
    private int modeType;
    private List<Contact> contacts;
    private int position;
    private String modifyContactAddress;
    ...
    public static void startContactsCreateActivity(Activity activity, int type, String firstName) {
        ...
        intent.putExtra(C_ADDRESS, firstName);
    ...
    private void initData() {
        ...
        modifyContactAddress = getIntent().getStringExtra(C_ADDRESS);
        contacts = ContactDaoUtils.sqlAll(MyApp.getContext());
    ...
    private void initView() {
        if (modeType == CREATE || StringUtil.isEmpty(modifyContactAddress)) return;
        Contact contact = null;
        if (contacts == null || contacts.size() <= 0) return;
        for (int i = 0; i < contacts.size(); i++) {
            if (modifyContactAddress.equals(contacts.get(i).getWalletAddress())) {
    ...
    private void addContacts() {
        ...
        if (!StringUtil.isEmpty(firstName)) contact.setFirstName(firstName);
        ...
        if (modeType == CREATE) {
            ContactDaoUtils.insertContact(MyApp.getContext(), contact);
        } else {
            ContactDaoUtils.updateContact(MyApp.getContext(), contact);
