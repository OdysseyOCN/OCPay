...
//        setLanguage(request);
        ...
        if (token == null || (!token.getScope().equalsIgnoreCase("ocpay") && !token.getScope().equalsIgnoreCase("member")))
            throw new UnauthorizedException("Please log in first");
        if (token.getScope().equalsIgnoreCase("ocpay"))
            request.setAttribute("adminId", token.getResourceOwnerId());
        if (token.getScope().equalsIgnoreCase("member"))
            request.setAttribute("memberId", token.getResourceOwnerId());
        return super.preHandle(request, response, handler);
    }
